...
This is the same convergence problem that the [pre-hash versus pure ML-DSA question](/blog/ml-dsa-pre-hash-or-pure) presents: the standard defines multiple modes but the ecosystem needs to pick one. Again, the right answer is the simpler one that eliminates unnecessary degrees of freedom. For secret key representation, use the seed.
