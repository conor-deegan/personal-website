...
It's called GITS, for **G**itHub **I**s **T**he **S**ignature, and it's [here](https://github.com/conor-deegan/gits). Don't use it for anything.
...
This is a Winternitz one-time signature, WOTS+ with `w=16`, from [Hülsing in 2013](https://eprint.iacr.org/2017/965.pdf). I didn't invent it, I just gave it a home as a git repository.
...
On calling it post-quantum. Grover's algorithm halves the exponent on a preimage search, and nothing better is known against a hash function, which is [why hash-based signatures survive](/blog/post-quantum-signatures-part-1). SHA-256 goes from 2^256 guesses to about 2^128 quantum operations, which is beyond plenty of security. SHA-1 would go to 2^80, which is possibly not (although this is debateable). Interestingly, a default git repo is not post-quantum, and it wasn't quite pre-quantum either.
