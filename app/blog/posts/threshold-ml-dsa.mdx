...
**Stage two, public, on the total.** Add up the `z{i}`s that passed and run the real ML-DSA checks on the combined `z`: the size check, and the commitment rounding check (Rule 2) that could only ever be answered by look at the total. This runs in the open, and it's safe, because every piece in the sum already cleared stage one and carries no secret. Nothing binned is ever shown; nothing shown is ever leaky.
