...
The result is a standard ML-DSA signature; same size, accepted by the same verifier as one a single signer would produce (though, as noted above, not statistically identical to it), created by a group where no one ever held the whole key, and secure even if all but one of them is compromised.
